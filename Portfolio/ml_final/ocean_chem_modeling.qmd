...
6.  `R_Sal` - Reported Salinity (from Specific Volume Anomoly, M³/Kg)
...
param_grid_tree = {
    'xgb__min_child_weight': range(1, 21, 2), # range(1, 21, 1),
    'xgb__max_depth': range(1, 21, 2), # range(1, 21, 1),
    'xgb__gamma': np.linspace(0, 1, 10), # np.linspace(0, 1, 20),
    'xgb__n_estimators': range(100, 1500, 50)  # adjust the number of trees np.linspace(100, 1500, 30) 
}
...
# make our cv grid
lr_grid <- expand.grid(learn_rate = seq(0.0001, 0.5, length.out = 100)) # seq(0.0001, 0.5, length.out = 200))
...
opt_trees <- as.numeric(show_best(fit_tune_trees, metric = "rmse")[1,4])
```
...
param_grid = {
    'svm__C': np.logspace(-5, 5, 50), # np.logspace(-5, 5, 100),
}
...
Our linear regression model performed the best in both Python and R. This is expected, given that our predictors are highly linearly correlated with the `DIC` variable. Our random forest regressor and support vector machine also performed quite well, which is sensible since these models also perform well with linearly correlated data.

One thing to note is that while we used RMSE to evaluate our models, this isn't the only metric that can be used for evaluation, and the best metric depends on the problem. RMSE is a common measure for regression models and is appropriate for our case where we are predicting a continuous variable. In future work, we could explore more feature engineering techniques, or combining our models into an ensemble.
